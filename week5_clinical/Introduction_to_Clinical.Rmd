...
```{r}
write.csv(clinic, "/home1/wjbrown/490_cluster/analysis_data/brca_clinical_data.csv", row.names=FALSE)
...
```{r}
clinic <- read.csv("/home1/wjbrown/490_cluster/analysis_data/brca_clinical_data.csv")
```
...
# 1. How many rows are there? How many columns?
  1097 rows, 112 columns
# 2. What is the data type of $gender?
  chr
# 3. What is the age in years of the 5th patient (barcode: TCGA-4H-AAAK)?
  50 in 2013 so around 61 now
# 4. How old was the 1st patient at their last follow up (barcode: TCGA-3C-AAAU)? HINT: you need to look at data from two columns to answer thi
  	20211 + 3767 = 23,978/365 = 65 yrs old
...
```{r Exercise 4.1}
# create plot here
plot(clinic$age_at_diagnosis, clinic$last_contact_days_to, col = "blue", ylab ="last contact days to", xlab = "age at diagnosis")
```
...
```{r Exercise 4.2}
# create plot here
boxplot(clinic$age_at_diagnosis ~ clinic$gender, xlab = "gender", ylab = "age_at_diagnosis", main = "gender to age at diagnosis", notch = FALSE)
```
...
```{r}
under30_over60_mask <- clinic$age_at_diagnosis < 34 | clinic$age_at_diagnosis >60
```
...
```{r}
cleaned_clinic$age_category <- ifelse(clinic$age_at_diagnosis <=35, "Young", ifelse(clinic$age_at_diagnosis <50, "Old", "Middle"))
```
...
cleaned_clinic$death_days_to <- as.numeric(cleaned_clinic$death_days_to)
...
head(cleaned_clinic$death_event)
```
