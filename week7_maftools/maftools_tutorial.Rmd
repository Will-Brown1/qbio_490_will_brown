...
```{r setup}
knitr::opts_knit$set(root.dir = normalizePath("/home1/wjbrown/490_cluster/analysis_data")) 
getwd()
```
...
```{r Exercise 1.1}
clinical <- read.csv("/home1/wjbrown/490_cluster/analysis_data/brca_clinical_data.csv")
```
...
ggsave("/Users/Will/Documents/QBIO490/qbio_490_will_brown/oncoplot_top#_BRCA.png")
```
...
genes <- c('TP53', 'BRCA1', 'BRCA2', 'CD47', 'TTN')
...
Hypothesized Reason: TTN is a lartge gene, as you get older it likely follows that there will be more mutations present. It is also considered a passenger mutation and occurs more frequently as cancer progresses.
...
It tells us that there's no statistically significant association between gender and vital status since this value is greater than 0.05, and it is likely that the differences could instead be due to chance.


Does this make sense given the biology of cancer?

Yes because even though some cancers might favor one gender or another, the biology for many cancers does not have gender play a major role in survival outcomes.
...
mut_pats_geneB <- geneB_maf@data$Tumor_Sample_Barcode
```
...
It tells us how many patients have mutations in both genes. Large lengths means many people and small means it rarely happens for both.
...
Young patients because mutated at 38.74% as opposed to old's 29.54%
- Are there more mutations in a specific region of the protein? Why might that be?
More in the p53 domain and P53 tetramer domain. This is likely because these domains are needed for DNA binding and tetramerization in order to supress tumors.
- What kinds of mutations are the most common?
Missense (single amino acid change)
- Are there any other interesting observations?
In the p53 tetramer domain there appears to be more nonsense and frameshift mutations then missense mutations. 
...
For the TP53 gene, it is a tumor supressor gene that helps in preventing cancer formation through cell division regulation, DNA repair, and apoptosis. It's relationship to survival is that mutations to this gene generally result in worse survival rates since patients with the mutation may lose their abilities to effectively regulate these biological processes.
What can you conclude from your plot?
From the plot we see that theere is less patient data with the mutation then the wildtype by about half. Those with the mutant group have slightly worse survival probabilities the the wild type. The p value of 0.0858 suggests that the observed differences in survival times between the wild and mutated is not completely conclusive if we go off of p values neededing to be below 0.05 for signifigance. The hazard ratio suggests a 42% higher risk for mutated patients then the wild ones.

Is this what you would expect according to the literature? Why or why not?
Somewhat -- since I would expect that the p value would indeed indicate a significance between survival for mutation and death. The hazard ratio does seem to be consistent since it is likely that someone with a mutated p53 gene would have a higher risk for death.
